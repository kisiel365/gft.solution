...
import com.google.common.collect.Sets;
 ...
 public class Scenario34 extends Scenario
 {
   public List<PositionOrder> orders()
   {
     return Arrays.asList(
       ...
       .details(OrderDetails.builder().amount(300).price(149).build()).build(), 
       PositionOrder.builder().id(8).timestamp(8L).client("107").broker("1").side(Side.SELL).product("D")
       .details(OrderDetails.builder().amount(400).price(99).build()).build() );
   }
   ...
   public Set<Transaction> transactions()
   {
     return Sets.newHashSet(new Transaction[] {
       ...
       Transaction.builder().id(1).amount(200).price(111).brokerBuy("2").brokerSell("3").clientBuy("101")
       .clientSell("105").product("B").build(), 
       Transaction.builder().id(1).amount(300).price(144).brokerBuy("2").brokerSell("3").clientBuy("106")
       .clientSell("102").product("C").build(), 
       Transaction.builder().id(1).amount(400).price(150).brokerBuy("4").brokerSell("1").clientBuy("103")
