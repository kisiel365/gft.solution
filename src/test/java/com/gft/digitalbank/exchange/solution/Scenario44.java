...
// refused
public class Scenario44 extends Scenario {
	public List<PositionOrder> orders() {
		return java.util.Arrays.asList(
				...
				PositionOrder.builder().id(3).timestamp(3L).client("102").broker("1").side(Side.SELL).product("A")
						.details(OrderDetails.builder().amount(300000).price(15).build()).build(),
				PositionOrder.builder().id(6).timestamp(6L).client("103").broker("1").side(Side.BUY).product("A")
						.details(OrderDetails.builder().amount(10000000).price(15).build()).build(),
				PositionOrder.builder().id(7).timestamp(7L).client("104").broker("3").side(Side.BUY).product("A")
						.details(OrderDetails.builder().amount(200000000).price(16).build()).build());
	}
	...
	public List<ModificationOrder> modifications() {
		return Arrays.asList(com.gft.digitalbank.exchange.model.orders.ModificationOrder.builder().id(5).timestamp(5L)
				.broker("1").modifiedOrderId(1).details(OrderDetails.builder().amount(200000000).price(16).build())
				.build());
	}
	...
	public Set<Transaction> transactions() {
		return Sets.newHashSet(new Transaction[] {
				Transaction.builder().id(1).amount(300000).price(15).brokerBuy("1").brokerSell("1").clientBuy("103")
						.clientSell("102").product("A").build(),
				Transaction.builder().id(2).amount(200000000).price(16).brokerBuy("3").brokerSell("1").clientBuy("104")
				...
				.newHashSet(
						new OrderBook[] {
								OrderBook.builder().product("A")
										.buyEntry(com.gft.digitalbank.exchange.model.OrderEntry.builder().id(1)
												.amount(9700000).price(15).client("103").broker("1").build())
										.build() });
	}
}
