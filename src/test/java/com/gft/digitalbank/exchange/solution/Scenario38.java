...
public class Scenario38 extends Scenario {
	public List<PositionOrder> orders() {
		return java.util.Arrays.asList(
				...
				PositionOrder.builder().id(6).timestamp(6L).client("104").broker("1").side(Side.BUY).product("A")
						.details(OrderDetails.builder().amount(350).price(100).build()).build());
	}
	...
	public Set<Transaction> transactions() {
		return Sets.newHashSet(new Transaction[] { Transaction.builder().id(1).amount(300).price(100).brokerBuy("1")
				.brokerSell("1").clientBuy("104").clientSell("102").product("A").build() });
	}
						...
						new OrderBook[] {
								OrderBook.builder()
										.product("A").buyEntry(com.gft.digitalbank.exchange.model.OrderEntry.builder()
												.id(1).amount(50).price(100).broker("1").client("104").build())
										.build() });
	}
}
