...
public class Scenario37 extends Scenario {
	public List<PositionOrder> orders() {
		return java.util.Arrays.asList(
				...
				PositionOrder.builder().id(6).timestamp(6L).client("103").broker("2").side(Side.SELL).product("A")
						.details(OrderDetails.builder().amount(100).price(100).build()).build());
	}
	...
	public Set<Transaction> transactions() {
		return Sets.newHashSet(new Transaction[] { Transaction.builder().id(1).amount(100).price(100).brokerBuy("1")
				.brokerSell("2").clientBuy("102").clientSell("103").product("A").build() });
	}
						...
						new OrderBook[] {
								OrderBook.builder()
										.product("A").buyEntry(com.gft.digitalbank.exchange.model.OrderEntry.builder()
												.id(1).amount(200).price(100).broker("1").client("102").build())
										.build() });
	}
}
