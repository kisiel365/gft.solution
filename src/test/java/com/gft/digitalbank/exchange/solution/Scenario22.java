...
public class Scenario22 extends Scenario {
	public List<PositionOrder> orders() {
		return java.util.Arrays.asList(
				...
				PositionOrder.builder().id(5).timestamp(5L).client("100")
						.broker("1").side(Side.BUY).product("A")
						.details(OrderDetails.builder().amount(100).price(105).build()).build());
	}

	@Override
	public List<CancellationOrder> cancellations() {
		return Arrays.asList(CancellationOrder.builder().id(4).timestamp(4L).cancelledOrderId(2).broker("2").build());
	}
	...
	public Set<OrderBook> orderBooks() {
		return Sets.newHashSet(new OrderBook[] { OrderBook.builder().product("A")
				.buyEntry(OrderEntry.builder().id(1).amount(100).price(105).broker("1").client("100").build())
				.buyEntry(OrderEntry.builder().id(2).amount(100).price(100).broker("1").client("100").build())
				.buyEntry(OrderEntry.builder().id(3).amount(300).price(100).broker("1").client("102").build())
				.build() });
	}
}
