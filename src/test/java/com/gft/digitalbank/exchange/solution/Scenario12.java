 ...
 public class Scenario12 extends Scenario
 {
   ...
   public Set<Transaction> transactions()
   {
     return Sets.newHashSet(new Transaction[] {
       ...
       Transaction.builder().id(2).amount(200).price(100).brokerBuy("1").brokerSell("3").clientBuy("102")
       .clientSell("103").product("A").build() });
   }
   ...
   public Set<OrderBook> orderBooks()
   {
     return Sets.newHashSet(new OrderBook[] {
       ...
       .buyEntry(OrderEntry.builder().id(1).amount(100).price(100).broker("1").client("102").build())
       .buyEntry(OrderEntry.builder().id(2).amount(200).price(90).broker("2").client("101").build())
       .build() });
   }
 }
