...
public class Scenario04 extends Scenario {
	...
	public List<PositionOrder> orders() {
		return Arrays.asList(new PositionOrder[] {
				...
				PositionOrder.builder().id(2).timestamp(2L).client("101").broker("2").side(Side.BUY).product("B")
						.details(OrderDetails.builder().amount(110).price(100).build()).build() });
	}
	...
	public Set<OrderBook> orderBooks() {
		return Sets
				.newHashSet(new OrderBook[] {
						OrderBook.builder().product("A")
								.sellEntry(OrderEntry.builder().id(1).amount(100).price(100).broker("1").client("100")
										.build())
								.build(),
						OrderBook.builder().product("B").buyEntry(
								OrderEntry.builder().id(1).amount(110).price(100).broker("2").client("101").build())
								.build() });
	}
