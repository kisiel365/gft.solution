...
public class Scenario09 extends Scenario {
	...
	public Set<OrderBook> orderBooks() {
		return Sets.newHashSet(new OrderBook[] { OrderBook.builder().product("A")
				...
				.buyEntry(OrderEntry.builder().id(2).amount(300).price(100).broker("1").client("102").build())
				.buyEntry(OrderEntry.builder().id(3).amount(200).price(90).broker("2").client("101").build())
				.buyEntry(OrderEntry.builder().id(4).amount(1000).price(90).broker("2").client("103").build())
				.build() });
	}
